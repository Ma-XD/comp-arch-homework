...
void write_file(const std::vector<double> & array, const std::string & file_name) {
    std::ofstream fout;
    fout.open(file_name);
    if (!fout.is_open()) {
        std::cerr << "Cannot open file \"" << file_name << "\"";
...
std::vector<double> not_parallel(const std::vector<double> & array) {
    std::vector<double> prefix(array.size());
...
std::vector<double> parallel(const std::vector<double> & array) {
    int n = array.size();
    int logn = get_log2(n);
    std::vector<double> temp (n * (logn + 1));
    temp[0] = 0;
...
int main(int argc, char *argv[]) {
    ...
    std::vector<double> array = read_file(input_file_name), prefix_np, prefix_p;
